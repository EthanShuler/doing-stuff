 ...
 *  its behavior is fixed to one template (shared pool, S–F tiers, one unranked
 *  shelf, no dates). */
...
export const emptyListDraft = (): ListDraft => ({ name: '', emoji: '', noun: '', shared: false })
 ...
 *
 * The singular noun is the whole trick — it templates every sentence on the
 * board ("Add a flavor") — so the modal previews what it produces rather than
 * explaining it.
 */
  ...
  return (
    <ModalShell opened={opened} onClose={onClose} title={isEditing ? 'Edit list' : 'New list'} size="sm">
      ...
      <Text fz={text.caption} c={colors.muted} mb={18} style={{ fontFamily: fonts.sans, fontStyle: 'italic' }}>
        Add a {noun}{draft.shared ? ' · one shared board' : ''}
      </Text>
